...
from eldorado_get_json import get_ua
from comp_class import IT_LOTS,UA
...
def load_ua():
    try:
        ua=get_ua()
        """u=UA(dict_code)
        u._IT_LOTS__sheet=pd.DataFrame()
        u._IT_LOTS__basa=pd.DataFrame(columns=u._IT_LOTS__cil)
        u.set_panda_set(ua)
        ubasa=u.it_basa()
        ubasa.to_json('load_form_providers/loads/ua_basa.json')"""
    except:
        print('loaded ua error but will be try load old assemly')
        #ubasa = pd.read_json('load_form_providers/loads/ua_basa.json')
        with open("load_form_providers/loads/ua_price.json", "r") as write_file:
            ua = json.load(write_file)

    pcassembly_ua=Pc_assembly.objects.get(name_assembly='ua_all')
    count_on = 0
    count_no = 0
    temp_comp_on=[]
    temp_comp_no=[]
    for x in ua:
        if comp_it.filter(name_computers=x).exists():
            temp_comp_on.append(ua[x]['name'])
        else:
            temp_comp_no.append(ua[x]['name'])
    for x in temp_comp_on:
        try:
            temp = Computers.objects.filter(name_computers=x,pc_assembly__sites__name_sites='ua')
            if temp.count() >= 1:
                te = temp.first()
                try:
    ...
    """for x in ubasa.iloc:
        ...
        if temp:
            ...
            if t and not Computers.objects.filter(name_computers=t['name_computers']):
                ...
                vent_num_computers=1,mem_num_computers=1,
                video_num_computers=1,pc_assembly=pcassembly_ua)
                c.save()
                count_no+=1"""
    print(f'ua loaded with {count_on} changed in computers and add {count_no} computers')
    with open("load_form_providers/loads/log.json", "r") as write_file:
        dict_log=json.load(write_file)
    dict_ua_log = {'time': datetime.datetime.today().strftime("%d-%m-%y,%H:%M"),
    'mes': f'ua loaded with {count_on} changed in computers and add {count_no} computers'}
    dict_log['dict_ua_log'] = dict_ua_log
