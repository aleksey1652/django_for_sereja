...
from cat.forms import ShortSearchForm,ArticleCreateForm,ShortCreateForm,ShortdeForm,FulldeForm,Pc_assemblyForm
...
def uploader(request,w):
    ...
    if request.method == 'POST' and 'itlink' in request.FILES:
        ...
        messages.success(request, f'From file it was add in db: {count_no} and update: {count_on} ')
    ...
    if w == 'update':
        ...
        messages.success(request, 'Price updating '+(timezone.now()+timezone.timedelta(hours=3)).strftime("%m/%d--%H:%M"))
    if w == 'fury':
        try:
            t = task_fury.delay()
        except:
        ...
        #res = task_trigger.delay(task_id=t.task_id,mask='DigitalFury') if t else None
        ...
        messages.success(request, 'Digitalfury updating '+(timezone.now()+timezone.timedelta(hours=3)).strftime("%m/%d--%H:%M"))
    if w == 'itblok':
        try:
            task_itblok.delay()
        except:
        ...
        messages.success(request, 'Itblok updating '+(timezone.now()+timezone.timedelta(hours=3)).strftime("%m/%d--%H:%M"))
    if w == 'versum':
        try:
            task_versum.delay()
        except:
        ...
        messages.success(request, 'Versum updating '+(timezone.now()+timezone.timedelta(hours=3)).strftime("%m/%d--%H:%M"))
    """if w == 'ua':
        try:
            task_ua.delay()
        except:
    ...
    if w == 'art':
        ...
        messages.success(request, 'Artline updating '+(timezone.now()+timezone.timedelta(hours=3)).strftime("%m/%d--%H:%M"))
    if w.find('result') != -1 :
...
def content_versum2(name_parts,kind):
    bit = {'content_versum2':True}
    if kind in ('soft','wifi','cables') and (name_parts == None or name_parts == ''):
        name_parts = 'empty_' + kind
    if name_parts == 'пусто':
        return ['','1'],bit
    if not isinstance(kind,tuple):
        short = Parts_short.objects.filter(name_parts=name_parts,kind=kind)
    else:
    ...
    if short.exists():
        ...
        if short2.parts_full.first():
            return [full2.partnumber_parts, short2.Advanced_parts], bit
        else:
            return ['','1'],bit
    else:
...
def content_for_google_sheet(obj,p):
    ...
    dict_atr = {'mb_computers':'mb', 'proc_computers':'cpu', 'mem_computers':'ram', 'video_computers':'gpu',
    'hdd_computers':'hdd', 'case_computers':'case', 'ps_computers':'psu', 'cool_computers':'cooler',
    'vent_computers':'fan','wifi_computers':'wifi','cables_computers':'cables','soft_computers':'soft'}
...
class versum_api(APIView):
    def get(self, request):
        ...
        for x in cver:
            ...
            Dict_full[x.name_computers] = co[:-1] + [['category', serie]] + [['desc_ru',desc_ru]] + [['desc_ukr', desc_ukr]]+[prom_list]+[['you_vid',you_vid]]+[['perm_conf',perm_conf]]+[['elite_conf',elite_conf]]
