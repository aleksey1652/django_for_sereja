...
def rivals_to_models(rivals_):
    ...
    if rivals_ == 'hotline':
        ...
        assemble_no_url =  Assemblage.objects.filter(hotline__isnull=True)
        assemble_no_url.update(hot_price=0, ver_hot=0) # затираем если нет ссылки
    ...
    if rivals_ == 'itblok':
        ...
        assemble_no_url =  Assemblage.objects.filter(itblok__isnull=True)
        assemble_no_url.update(it_price=0, ver_it=0) # затираем если нет ссылки
        for asml in assemble:
    ...
    if rivals_ == 'artline':
        ...
        assemble_no_url =  Assemblage.objects.filter(artline__isnull=True)
        assemble_no_url.update(art_price=0, ver_art=0) # затираем если нет ссылки
        for asml in assemble:
    ...
    if rivals_ == 'telemart':
        ...
        assemble_no_url =  Assemblage.objects.filter(telemart__isnull=True)
        assemble_no_url.update(tel_price=0, ver_tel=0) # затираем если нет ссылки
        for asml in assemble:
    ...
    if rivals_ == 'compx':
        ...
        assemble_no_url =  Assemblage.objects.filter(compx__isnull=True)
        assemble_no_url.update(com_price=0, ver_com=0) # затираем если нет ссылки
        for asml in assemble:
