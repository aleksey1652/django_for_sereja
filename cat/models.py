...
def procent_to_payment(procent = F('class_computers')):
    # простая ф: получаем на входе нацнку, а выводим кол платежей для модели компов
...
class USD(models.Model):
    ...
    for_json_dump = models.BooleanField(default=True,
    verbose_name='вкл/выкл json_dump')
...
def result_saves(sender, instance, created, **kwargs):
    # после сохр пересчитываем кол платежей (payment) для компов (при 0 индивид наценке)
    # используя наценку из USD
    margin_ = instance.margin
    pay = procent_to_payment(procent=margin_)
    comps = Computers.objects.filter(pc_assembly__sites__name_sites='versum').exclude(
    pc_assembly__kind_assembly='Other'
    ) # все версум компы кроме амд, интел калькулятора!!!
    comps_default_margin = comps.filter(class_computers=0)
    #comps_notnull_margin = comps.filter(class_computers__gt=0)
    comps_default_margin.update(payment=pay)
    #comps_notnull_margin.update(
    #payment=procent_to_payment()
    #)
...
class Parts_full(models.Model):
    ...
    def get_providers(self):
        # совмещаем name_parts_main и providers

        first_prv = self.name_parts_main # пров (1поставщик)
        prv = self.providers.name_provider # пров результирующий
        return  f'- {first_prv}' if first_prv else f' {prv}'

    get_providers.short_description = 'пост-1пост'
...
class Parts_short(models.Model):
    ...
    def sklad_view(self):
        try:
            ...
            providerprice_parts__gt=0).filter(
            remainder__isnull=False).order_by('providerprice_parts'
            ).first().remainder)[0]))
        except:
            return '-'
...
class Computers(models.Model):
    ...
    proc_computers = models.CharField(max_length=300, db_index=True,
    ...
    payment = models.PositiveIntegerField(default=0, verbose_name='Платежи')
...
class Sites(models.Model):
    ...
    name_sites =  models.CharField(max_length=50, db_index=True,
    choices=CHOISE,default='itblok', verbose_name='Name_sites')
...
class Promotion(models.Model):
    prom = models.CharField(max_length=300, db_index=True, verbose_name='Ярлык',
    default='пусто')
    english_prom = models.CharField(null=True,blank=True,max_length=300, db_index=True,
    verbose_name='Ярлык(англ)', default='empty')
...
class Pc_assembly(models.Model):
    ...
    verbose_name='Серии')
    matrix = models.PositiveIntegerField(default=1, verbose_name='Группы+')
