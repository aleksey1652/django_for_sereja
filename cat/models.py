...
class Parts_short(models.Model):
    ...
    def description_view(self):
        ...
        try:
            ...
            descr = None
        try:
            descr = descr.filter(is_active=True).first()
        except:
            descr = None
...
class Computers(models.Model):
    ...
    default=1.17, verbose_name='Наценка')
    ...
    def comp_name_plus(self):
        ...
        if self.case_computers.lower().find('white') != -1:
            return f'{self.name_computers}---({proc}-{gpu}-{mem}-{mb}-white)'
        return f'{self.name_computers}---({proc}-{gpu}-{mem}-{mb})'
...
class Pc_assembly(models.Model):
    name_assembly = models.CharField(max_length=300, db_index=True, verbose_name='Группа')
