...
class ComputersForm(ModelForm):
    ...
    def clean_class_computers(self):
        class_computers = self.cleaned_data.get('class_computers')
        if class_computers is not None and class_computers < 0:
            raise ValidationError('наценка < 0')
        return class_computers
