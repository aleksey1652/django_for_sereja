...
from load_form_providers.load_element import to_article2_1, get_distrib2
        ...
        if form.is_valid():
            ...
            if not p2.count():
                ...
                if not price:
                    ...
                    'mti','brain','edg')).order_by('providerprice_parts')
                    st = 'no'
                    for p_p in  p_prov:
...
def pc_assembly_page_advanced(request, page_id, price, grp):
    if request.method == 'GET':
        ...
        dict_parts={'aproc':Acpu,'iproc':Icpu,'video':Vid,'imb':Imb,'amb':Amb,'mem':Mem,'ps':Ps,'case':Case,'ssd':Ssd,'hdd':Hdd}
    ...
    if request.method == 'POST':
        ...
        kind_dict = {'aproc':'proc_computers','iproc':'proc_computers','amb':'mb_computers',
                     ...
                     'ssd':'hdd_computers','hdd':'hdd_computers'}
        p = Parts_short.objects.get(pk=int(page_id))
        try:
        ...
        for x in request.POST:
            if x != 'csrfmiddlewaretoken':
                try:
                    ...
                    continue
                if p.kind in ('aproc','iproc','imb','amb','video','mem','ps','case'):
...
def pc_assembly_page(request, page_id):
    ...
    if request.method == 'GET':
        ...
        if page_id == 'itblok':
            ...
            series_list = []
            for s in set_series:
                series_list.append(Pc_assembly.objects.filter(sites__name_sites='versum',kind_assembly=s))
