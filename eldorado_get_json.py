...
def get_itblok():
    ...
    try:
        for m in range(1,10):
            list1=[]
            try:
                g=get_html(f'https://it-blok.com.ua/computeri.html/?page={m}')
            except:
            ...
            for n in list1:
                try:
                    if len(n.keys()) == 5:
                        ...
                        Reverse_param={"Процессор":'proc',"На чипсете":'mb',
                        "Мощность":'ps',
                        "Корпус":'case',"Кулер для процессора":'cool'}
                        lis_par=["Процессор","На чипсете","Мощность","Корпус","Кулер для процессора"]
                        temp_hdd_ssd = []
                        temp_mem = []
                        temp_video = []
                        for x in sall:
                            for y in x.findAll('tr'):
                                ...
                                elif to_dict in ("Объем HDD", "Диск SSD","SSD M.2 NVMe"):
                                    try:
                                        temp_hdd_ssd.append(to_dict[-3:].lower() + ' ' +  y.find_all('td')[1].text)
                                    except:
                                        temp_mem.append('')
                                elif to_dict in ("Объем памяти ОЗУ", "Тип памяти"):
                                    try:
                                        temp_mem.append(y.find_all('td')[1].text)
                                    except:
                                        temp_mem.append('')
                                elif to_dict in ("Видеокарта", "Объем видеопамяти"):
                                    try:
                        ...
                        try:
                            ...
                            n['mem'] = ''
                        try:
                            n['video'] = temp_video[0] + ' ' +  temp_video[1] if len(temp_video) == 2 else temp_video[0]
                        except:
                            n['video'] = ''
                    try:
                        comp_dict[n['name']] = n
