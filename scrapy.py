...
import re
import pandas as pd
from money.service import get_art_single_comp, cpu_to_code, video_to_code, mem_to_code, test_art_comp
...
def get_html(url):
    response = requests.get(url,proxies=None,timeout=5,headers=random_headers())
...
def get_soup_art(url):
    ...
    #list_error = []
    for m in range(1,96):
        g = get_html(url+f'/catalog/kompyutery-artline/page={m}')
        if g:
            soup = BeautifulSoup(g, "html.parser")
            ...
            if comps:
                for comp in comps:
                    if isinstance(comp, bs4.element.Tag) and comp.has_attr('data-url'):
                        ...
                        """else:
                            list_error.append(url_comp)"""

    #return (dict_art, {'url_list': url_list}, {'list_error': list_error})
    return dict_art
