...
def personal_kurier(request, now, context, st_pk):
    ...
    person = request.user # HttpResponseRedirect
    M = Managers.objects.get(family__pk=person.pk)

    formK = StatsDescrForm()
    context['formK'] = formK
    ...
    context['st_period'] = sorted_dates_str
    if st_pk != 0:
        m_y = re.split(':',st_pk)
        try:
            month, year = m_y[0].strip(), m_y[1].strip()
        ...
        if len(m_y) == 3:
            ...
            #form = StatsDescrForm(initial={'description': name_ss.description})
            #context['form_st'] = form
        messages.success(request,f'Период (мес):{st_pk}')
        #kurier_periods(request, f'{month}: {year}', 1)
