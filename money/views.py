    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if comp.first().pc_assembly.kind_assembly == 'Other':
                ...
                pay = procent_to_payment(procent=margin)
                comp.update(class_computers=margin, payment=pay)
                messages.success(
                request,f'В выбранных компах наценка теперь: {margin}, платежей: {pay}')
            return  HttpResponseRedirect(
...
def personal_one_c(request, now, context, st_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            service,_ = Service.objects.get_or_create(sloznostPK=service_sloznostPK,
            kind='Оператор 1С')
    ...
    if gr == 'test_group':
        ...
        if request.method == 'POST':
            ...
            if form_bonus.is_valid():
                ...
                serv, _ = Service.objects.get_or_create(kind='Офисный менеджер',
                sloznostPK='простой')
                month, year = date_field.strftime("%m"), date_field.strftime("%Y")
        ...
        #sal = Salary(month, year)
        context['manager_stats_per_period'] = manager_stats_per_period(M,
        bids_per_period, month, year)
        w1, w2 = 0, 0
...
def save_create_data(data, only_sborka=False, only_remont=False):
    ...
    if only_remont == True and data['status'] in st:
        ...
        istocnikZakaza=ch_statusdata['istocnikZakaza'],
        managers=man
        )
