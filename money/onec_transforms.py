...
from cat.models import One_C, Providers, Parts_full, USD
import re
...
def get_avg_check(site_, price_, month, year, now):
    ...
    if not Average_check.objects.filter(date__month=month,date__year=year, site=site_).exists():
        if Average_check.objects.filter(site=site_).exists():
            ...
            average_check_versum = Average_check.objects.create(site=site_,
            quantity=avg_last.quantity,
        ...
        avg = Average_check.objects.filter(date__month=month,date__year=year,
        site=site_).last()
        avg.price = price_
...
def transformations(data_one):
    ...
    usd_j = USD.objects.all().last()
    if usd_j.for_json_dump:
        # шоб не захламлять и в админке вкл/выкл при необходимости
        ...
        for e in data_one['remainder']:
            try:
                ...
                if Parts_full.objects.filter(partnumber_parts=product_id, providers__name_provider='-').exists():
                    full = Parts_full.objects.filter(partnumber_parts=product_id,
                    providers__name_provider='-')
                    full.filter(availability_parts='yes').update(
                    remainder=f"price:{cost}; {e['quantity']}", date_chg=now)
                    full.filter(availability_parts__in=('no',
                    'q')).update(remainder=f"price:{cost}; {e['quantity']}",
