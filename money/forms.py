...
class ServiceForm(forms.Form):
    ...
    try:
        managers = Managers.objects.all().values_list(
        'name',flat=True)
        CHOISE = [(man, man) for man in managers]
    except:
        CHOISE = [('пусто', 'пусто')]

    service_manager = forms.ChoiceField(
        choices=CHOISE,
        label='менеджер'
    )
    ...
    def clean_active(self):
        # получаем (преобразовуем) поле active в булево значение иначе str "True"
