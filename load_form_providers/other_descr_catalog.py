...
def category_re(str_):
    k = "миша|крісло|клавіатура|комплект|гарнітура|навушники|акустична система|комп'ютерний стіл|килимок|поверхня|веб камера|мікрофон"
    try:
    ...
    def from_mti(self, filename):
        ...
        for x in result_data:
            ...
            for data in list_temp:
                ...
                try:
                    for k in data.getchildren():
                        if k.tag in ['partnum', 'price_uah', 'store',
                        'productgroup_id', 'rrp']:
                            temp[k.tag] = k.text
