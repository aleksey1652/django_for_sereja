...
class DC:
    ...
    def get_sort_basa2(self):
        res=self.get_basa()
        if not res.empty:
            res2=res.rename(columns={'Article':'partnumber_parts', 'Name':'name_parts', 'PriceUSD':'providerprice_parts',
            'Availability':'availability_parts', 'Code':'images', 'Url':'url_parts', 'CategoryID':'subcategory',
            'RRP_UAH':'RRP_UAH'})
            return res2
        return pd.DataFrame()
'Article', 'CategoryID', 'Name',
