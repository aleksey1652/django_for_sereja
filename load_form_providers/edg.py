from load_form_providers.get_service import download_file,get_from_xml_edg,get_from_xml_edg2
...
class EDG:
    ...
    def get_basa(self, fun2=0):
        ...
        price_columns = [
            'Code', 'Name', 'Price', 'StockName', 'Images', 'Url', 'Subcategory'
        ]
        """price_columns = [
            'partnumber_parts', 'name_parts', 'providerprice_parts',
            'availability_parts', 'images', 'url_parts', 'subcategory'
        ]"""

        if not ff:
            r = requests.get(self.price_url)
            data,index = get_from_xml_edg2(r)
            basa = pd.DataFrame(data, columns=price_columns, index=index) if r.status_code == 200 else pd.DataFrame()
            return basa
    ...
    def get_sort_basa2(self):
        ...
        if not res.empty:
            ...
            'StockName':'availability_parts', 'Images':'images', 'Url':'url_parts', 'Subcategory':'subcategory'})
            return res2
