...
def get_from_json_elko2(r, r2,usd):
    try:
        ...
        usd = 1
    catalog = ('COC','CAS','LC3','MEM','WRA', 'MBA', 'HDS', 'VGP', 'PSU', 'SSM', 'MBI', 'SSU', 'CPU')
    try:
        elko = r.json()
    except:
    ...
    try:
        for x in elko:
            ...
            try:
                for k in x.keys():
                    if k in ['manufacturerCode', 'name', 'price', 'quantity', 'httpDescription', 'id', 'catalog']:
                        if not x[k]:
                            di[k] = 'None'
                        else:
                            if k != 'name':
            ...
            for r in list_temp:
                ...
                try:
                    for k in r.getchildren():
                        if k.tag in [
                            'prodname', 'partnum', 'price_uah', 'store', 'productgroup_id', 'mticode'
                                    ]:
                            if not k.text:
                                di[k.tag] = 'None'
                            else:
...
def get_from_xml_edg2(r):
    Subcategory = ('Блоки питания ATX','Корпуса', 'Системы охлаждения, Cooler')
    ...
    try:
        ...
        for x in rr:
            ...
            try:
                for k in x.getchildren():
                    if k.tag in ['Code', 'Name', 'Price', 'StockName', 'Images', 'Subcategory']:
                        if k.tag == 'Images':
                            if k.getchildren():
                                temp_list = []
...
def get_from_xml_asbis2(content):
