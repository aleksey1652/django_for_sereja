from load_form_providers.get_service import download_file,get_from_xml_mti,price_test,get_from_xml_mti2
...
class MTI:
    ...
    def __data(self, store, ff=0):
        ...
        if not ff:
            data, index = get_from_xml_mti2(r)
            return pd.DataFrame(data, columns=price_columns, index=index) if data else pd.DataFrame()
    ...
    def get_sort_basa2(self):
        ...
        if not res.empty:
            for x in res.iloc:
                if not x.empty:
                    x.price_uah = round(float(price_test(x.price_uah))/self.usd_ua, 1)
            ...
            'mticode':'images', 'url':'url_parts', 'productgroup_id':'subcategory'})
            return res2
    ...
    """def get_sort_basa2(self):
        ...
        for x in res.iloc:
            if not x.empty:
                x.price_uah_order = round(float(price_test(x.price_uah_order))/self.usd_ua, 1)
                list_mti.append(x)
            else:
                list_mti.append(pd.DataFrame())
        return list_mti"""
