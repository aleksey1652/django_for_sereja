...
class BRAIN:
    ...
    def __get_from_brain(self, filename, ff):
        ...
        for x in j.keys():
            ...
            for k,v in j[x].items():
                if k in ['Article', 'Name', 'PriceUSD', 'Available', 'URL', 'Group']:
                    d[k]=v
            if ff:
    ...
    def __get_from_brain2(self, filename):
    ...
    def __board2(self, s, t):
        dop = '?lang=ru&full=1'
        if t['targetID'] in (4,5,29):
            r = requests.get(f"http://api.brain.com.ua/pricelists/{t['targetID']}/json/{s}{dop}")
            if r.json()['status']:
                filename = download_file(r.json()['url'])
    ...
    def get_basa(self, fun2=0):
        ...
        if not ff:
            ...
            for t in self.__tt(s):
                try:
                    l_data, l_index = self.__board2(s, t)
                except:
                    pass
                    continue
                if l_data and l_index:
                    l_full_data+=l_data
    ...
    def get_sort_basa2(self):
        ...
        if not res.empty:
            ...
            'URL':'url_parts', 'Group':'subcategory'})
            return res2
